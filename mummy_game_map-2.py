...
# Load hình bẫy (hoặc dùng màu đỏ)
TRAP_IMG = pygame.image.load("./assets/trap.png")
TRAP_TILE = pygame.transform.scale(TRAP_IMG, (GO_DISTANCE, GO_DISTANCE))
    ...
    def update(self, left, down, up, right):
        if GameManager.mummy_move == 0:
            if self.go < GO_DISTANCE:
                ...
                # Cập nhật rect để bóp tọa độ về lưới
                self.update_rect()
                
                # --- KIỂM TRA TRAP NGAY Ở ĐÂY ---
                player_key = get_key(self.x, self.y)
                if player_key in TRAP_KEYS:
    ...
    def run(self, key_player, player_x, player_y):       
        ...
        if self.y != self.run_pos["Y"]:
            ...
            if key_mummy == key_player:
                ...
                # block chữ L
                if key_mummy == 20 and player_y > self.y and player_x > self.x:
                    # Player nằm dưới và nằm trong cột tường dọc
                    GameManager.mummy_move      = 0
                    GameManager.can_player_move = True               
                # Đk1: mummy đang đúng bên phải cái tường, Đk2: # mummy đang đúng bên trái cái tường
                ...
                elif (key_mummy == 21 and player_x < mummy.x) or (key_mummy == 21 - 1 and player_x > mummy.x): 
...
wall_v_21 = Wall(21, RED)
wall_v_21.setup("vertical")
...
while running:
    ...
    # Vẽ tất cả các ô trap bằng TRAP_TILE
    for k in TRAP_KEYS:
        pos = get_pos(k)
        DISPLAY.blit(TRAP_TILE, (pos['X'], pos['Y']))
    
    # Setup wall    
    wall_h_26.draw("horizontal")
    wall_v_21.draw("vertical")

    # Setup ô chiến thắng
    DISPLAY.blit(stairs_down, (WIN_POS['X'], WIN_POS['Y']))
